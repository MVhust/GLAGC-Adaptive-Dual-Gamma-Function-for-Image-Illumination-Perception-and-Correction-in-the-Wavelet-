input_img=imread('C:\Users\23991\Desktop\论文\final_exp\imgs\9.BMP');
size(input_img)
input_img=  imresize(input_img,[416,416]);
...
LH=LH.*power(I_max,1-gama).*gama.*power(LL+0.01,gama-1); 
HL=HL.*power(I_max,1-gama).*gama.*power(LL+0.01,gama-1);
HH=HH.*power(I_max,1-gama).*gama.*power(LL+0.01,gama-1);  % 线性抑制，+0.01是为了防止分母出现0
